...
function eveMethod(spec) {
  ...
  return function() {
    ...
    var requestPath = commandPath
    if(requestParams) requestPath = requestPath + '?' + requestParams;                                                
    ...
    cache.read(cacheKey, function(err, data) {
      ...
      self._request(requestMethod, requestPath, opts.auth, options, requestCallback);
