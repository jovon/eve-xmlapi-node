...
function EveResource(eve, urlData) {
  ...
  // this.basePath = utils.makeURLInterpolator(eve.getApiField('basePath'));
  ...
  */
        ...
        parseString(response, function(err, result){
          try {
            ...
            if (result.eveapi.error) {     
              ...
              if (errorCode === '106') {
                err = new Error.EveAuthenticationError(errorMessage);
              } else if (res.statusCode === 429) {
                err = new Error.EveRateLimitError(errorMessage);
              } else {
                err = Error.EveError.generate(errorMessage);
              }
  ...
  _request: function(method, path, data, auth, options, callback) {
    ...
    var headers = {      
      ...
      'User-Agent': 'EveApi/v1 NodeBindings/' + this._eve.getConstant('PACKAGE_VERSION'),
    };
