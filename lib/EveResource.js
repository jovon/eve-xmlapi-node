  ...
  _request: function(method, path, data, auth, options, callback) {
    ...
    if (self.requestDataProcessor) {
      ...
      if(requestData instanceof Error) {
        requestData = ''
        return callback(new Error.EveInvalidRequestError("Error with request query parameters"))
      }
    } else {
    ...
    var contentLength = requestData ? requestData.length : 0
    ...
    function makeRequest() {
      ...
      if (requestData) path = path + '?' + requestData
      var opt = {
        host: host,
